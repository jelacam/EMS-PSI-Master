...
namespace CIMAdapterTest.ImporterTest
{
    ...
    public class EMSConverterTest
    {
        ...
        public void SetupTest()
        {
            ...
            rdFuel = new ResourceDescription();
        ...
        [Test]
        [TestCase("1005", "TestRegulatingCondEq", TestName = "PopulateRegulatingCondEqPropertiesTest")]
        ...
        [Test]
        [TestCase("1006", "TestRegulatingCondEq", 10, TestName = "PopulateRotatingMachinePropertiesTest")]
        ...
        [Test]
        [TestCase("1007", "TestSynchronousMachine", 10, EMS.EmsFuelType.coal, EMS.SynchronousMachineOperatingMode.generator,
            10, 10, true, 80, (float)-0.8, (float)0.8, TestName = "PopulateSynchronousMachineePropertiesTest")]
        public void PopulateSynchronousMachinePropertiesTest(string mRid, string name, float ratedS, EMS.EmsFuelType fuelType,
            EMS.SynchronousMachineOperatingMode operatingMode, float maxQ, float minQ, bool active, float load, float minPhi, float maxPhi)
        {
            ...
            cimSynchronousMachine.Active = active;
            cimSynchronousMachine.LoadPct = load;
            cimSynchronousMachine.MaxCosPhi = maxPhi;
            cimSynchronousMachine.MinCosPhi = minPhi;
            ...
            Assert.AreEqual(rdSynchronousMachine.GetProperty(ModelCode.SYNCHRONOUSMACHINE_ACTIVE).AsBool(), active);
            Assert.AreEqual(rdSynchronousMachine.GetProperty(ModelCode.SYNCHRONOUSMACHINE_LOADPCT).AsFloat(), load);
            Assert.AreEqual(rdSynchronousMachine.GetProperty(ModelCode.SYNCHRONOUSMACHINE_MAXCOSPHI).AsFloat(), maxPhi);
            Assert.AreEqual(rdSynchronousMachine.GetProperty(ModelCode.SYNCHRONOUSMACHINE_MINCOSPHI).AsFloat(), minPhi);
        }

        [Test]
        [TestCase("1008", "TestMeasurement", "Napon", EMS.UnitSymbol.VA, TestName = "PopulateMeasurementPropertiesTest")]
        ...
        [Test]
        [TestCase("1009", "TestAnalog", 10, 10, 10, EMS.SignalDirection.Read, TestName = "PopulateAnalogPropertiesTest")]
        ...
        [Test]
        [TestCase("1010", "TestEmsFuel", EMS.EmsFuelType.coal, (float)60, TestName = "PopulateEmsFuelPropertiesTest")]
        public void PopulateEmsFuelPropertiesTest(string mRid, string name,EMS.EmsFuelType fuelType,float unitPrice)
        {
            cimFuel.MRID = mRid;
            cimFuel.Name = name;
            cimFuel.FuelType = fuelType;
            cimFuel.UnitPrice = unitPrice;
            
            EMSConverter.PopulateEmsFuelProperties(cimFuel, rdFuel, importHelper, report);
            Assert.AreEqual(rdFuel.GetProperty(ModelCode.IDENTIFIEDOBJECT_MRID).ToString(), mRid);
            Assert.AreEqual(rdFuel.GetProperty(ModelCode.IDENTIFIEDOBJECT_NAME).ToString(), name);
            Assert.AreEqual(rdFuel.GetProperty(ModelCode.EMSFUEL_FUELTYPE).AsEnum(), 0);
            Assert.AreEqual(rdFuel.GetProperty(ModelCode.EMSFUEL_UNITPRICE).AsFloat(), unitPrice);
        }
