...
namespace EMS.Common
{
    ...
    public class ModelResourcesDesc
    {
        ...
        private void Initialize()
        {
            ...
            foreach (EMSType t in this.allEMSTypes)
            {
                ...
                if (!this.type2modelCode.ContainsKey(type) && type != EMSType.MASK_TYPE)
                {
        ...
        public static List<EMSType> GetLeavesForCoreEntities(ModelCode entityType)
        {
        ...
        public List<EMSType> GetLeaves(ModelCode entityType)
        {
            List<EMSType> children = new List<EMSType>();
        ...
        public void GetResourceAncestors(ModelCode typeId, ref List<ModelCode> ancestors)
        {
            ...
            if (typeIdLong == (long)ModelCode.IDENTIFIEDOBJECT_GID)
            {
                return;
            }
        ...
        public ModelCode GetModelCodeFromType(EMSType type)
        {
            ModelCode modelCode;
        ...
        public bool TypeHasModelCode(EMSType type)
        {
        ...
        {
            EMSType type = (EMSType)ModelCodeHelper.ExtractTypeFromGlobalId(id);
        ...
        private ModelCode GetModelCodeFromTypeForCore(EMSType type)
        {
        ...
        private List<EMSType> SwitchShortsToDMSTypes(List<short> shortValues)
        {
            List<EMSType> result = new List<EMSType>();

            if (shortValues != null)
            {
                foreach (long value in shortValues)
                {
                    result.Add((EMSType)value);
                }
            }
        ...
        private List<short> SwitchDMSTypesToShorts(List<EMSType> modelCodeValues)
        {
            List<short> result = new List<short>();

            if (modelCodeValues != null)
            {
                foreach (EMSType value in modelCodeValues)
                {
    ...
    public class EMSTypeComparer : IEqualityComparer<EMSType>
    {
        ...
        public bool Equals(EMSType x, EMSType y)
        {
            return (short)x == (short)y;
        }

        public int GetHashCode(EMSType obj)
        {
            return (int)obj;
        }
